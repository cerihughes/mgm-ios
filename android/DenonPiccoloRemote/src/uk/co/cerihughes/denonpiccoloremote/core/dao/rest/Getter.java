...
public abstract class Getter<ResponseType>
{
	...
	public ResponseType get(String url) throws GetterException, ConvertException
	{
		String response = getStringResponse(url);
		return convertStringResponse(response);
	}

	protected abstract ResponseType convertStringResponse(String response) throws ConvertException;
	...
	private String getStringResponse(String url) throws GetterException
	{
		...
		try
		{
			...
			{
				throw new GetterException(String.format("Unexpected response [%d] for url [%s].", statusCode, url));
			}
		}
